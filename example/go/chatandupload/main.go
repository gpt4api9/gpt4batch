...
func main() {
	...
	chatReq := gpt4batch.OpenaiChatRequest{
		GizmoId:     "g-HMNcP6w7d",
		Message:     "你是gpt3还是gpt-4",
		Model:       "gpt-4",
		Attachments: gpt4batch.Attachments{result.Attachment},
		Parts:       gpt4batch.Parts{result.Part},
	}

	// 建议对话设置8分钟
	chatResp, err := resty.New().
	...
	var chatResult gpt4batch.ChatResponse
	if err := json.Unmarshal(chatResp.Body(), &result); err != nil {
		panic(err)
	}

	fmt.Println("<ChatResponse>: ", chatResult)
}
